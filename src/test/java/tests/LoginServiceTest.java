...
public class LoginServiceTest {
    ...
    private void createRequestDate(String userId, String userPw) {
    ...
    void loginSuccess() {
        createRequestDate(member.getUserId(), member.getUserPw());
        assertDoesNotThrow(() -> {
    ...
    void requiredFieldCheck() {
        assertAll(
                () -> {
                    // 아이디 검증
                    createRequestDate(null, member.getUserPw());
                    fieldEachCheck(request, "아이디");

                    createRequestDate("   ", member.getUserPw());
                    fieldEachCheck(request, "아이디");

                }, () -> {
                    // 비밀번호 검증
                    createRequestDate(member.getUserId(), null);
                    fieldEachCheck(request, "비밀번호");

                    createRequestDate(member.getUserId(), "   ");
                    fieldEachCheck(request, "비밀번호");
                });
